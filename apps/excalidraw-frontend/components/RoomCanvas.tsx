...
import { Home, DoorOpen, X, Users } from "lucide-react";
import { tokenManager } from "@/utils/tokenManager";
import { ConnectionStatus } from "./LoadingSpinner";
...
export function RoomCanvas({ roomId }: { roomId: string }) {
  ...
  const [confirmAction, setConfirmAction] = useState<"home" | "changeRoom" | null>(null);
  ...
  const handleChangeRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!validateResponse.ok) {
        setChangeRoomError(validationData.message || "Failed to validate room");
  ...
  if (connectionState !== "connected") {
    return (
      <ConnectionStatus
        ...
        onRetry={connectionState === "error" || connectionState === "disconnected" ? handleRetry : undefined}
      />
    );
  }
      ...
      {onlineUsers.length > 0 && (
        <div className="absolute top-4 right-4 z-50">
          <div className="flex items-center gap-2 bg-gray-900/90 backdrop-blur-sm border border-gray-700 rounded-xl px-3 py-2 shadow-lg">
            ...
            <span className="text-sm text-gray-300">{onlineUsers.length + 1} online</span>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm mx-4">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {confirmAction === "home" ? "Leave Canvas?" : "Change Room?"}
            </h3>
            ...
            <div className="flex gap-3">
              ...
              >
                Yes, {confirmAction === "home" ? "Go Home" : "Change Room"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-sm mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">Join Another Room</h3>
              <button
                onClick={() => {
            ...
            {changeRoomError && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-red-600 text-sm">{changeRoomError}</p>
              </div>
            )}
            ...
            <form onSubmit={handleChangeRoom} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewRoomData((prev) => ({ ...prev, roomId: e.target.value }))}
